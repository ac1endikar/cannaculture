...
for strain in white_label_strains:
    s_id = strain["id"]
    if f'id: "{s_id}"' not in text and f'id: \'{s_id}\'' not in text:
        formatted = ",\n  " + json.dumps(strain, indent=4, ensure_ascii=False).replace('\n', '\n  ')
        target = text.rfind("];")
        if target != -1:
            text = text[:target] + formatted + "\n" + text[target:]
            added_count += 1
            print(f"✅ Added {s_id} ({strain['name']})")

if added_count > 0:
    with open(data_path, 'w', encoding='utf-8') as f:
        f.write(text)
    print(f"\nSaved {added_count} White Label Seed Co. strains to data.js")
