...
from fetch_soma_seeds import SOMA_SEEDS_CATALOG
...
for s_strain in SOMA_SEEDS_CATALOG:
    s_obj = {k: v for k, v in s_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded Soma Seeds strain: {s_id} ({s_obj['name']})")

print(f"Total strains after Soma Seeds integration: {len(strains_map)}")
...
print(f"✅ Saved Soma Seeds database to data.js ({len(final_code):,} bytes)")
