...
from fetch_in_house_genetics_catalog import IN_HOUSE_GENETICS_CATALOG
...
for ihg_strain in IN_HOUSE_GENETICS_CATALOG:
    s_obj = {k: v for k, v in ihg_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded In-House Genetics strain: {s_id} ({s_obj['name']})")

print(f"Total strains after In-House Genetics integration: {len(strains_map)}")
...
print(f"✅ Saved In-House Genetics database to data.js ({len(final_code):,} bytes)")
