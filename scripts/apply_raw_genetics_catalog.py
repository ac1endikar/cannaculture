...
from fetch_raw_genetics_catalog import RAW_GENETICS_CATALOG
...
for raw_strain in RAW_GENETICS_CATALOG:
    s_obj = {k: v for k, v in raw_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded Raw Genetics strain: {s_id} ({s_obj['name']})")

print(f"Total strains after Raw Genetics integration: {len(strains_map)}")
...
print(f"✅ Saved Raw Genetics database to data.js ({len(final_code):,} bytes)")
