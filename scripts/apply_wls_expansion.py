...
# Import FULL_WLS_CATALOG list from expand_white_label_catalog.py
from expand_white_label_catalog import FULL_WLS_CATALOG

# Read existing strains from text
...
# Embed or update all White Label catalog strains
for w_strain in FULL_WLS_CATALOG:
    # Clone strain object without query field
    s_obj = {k: v for k, v in w_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded WLS strain: {s_id} ({s_obj['name']})")

print(f"Total strains after WLS expansion: {len(strains_map)}")
...
print(f"✅ Saved expanded database to data.js ({len(final_code):,} bytes)")
