...
from fetch_compound_genetics_catalog import COMPOUND_GENETICS_CATALOG
...
for cpg_strain in COMPOUND_GENETICS_CATALOG:
    s_obj = {k: v for k, v in cpg_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded Compound Genetics strain: {s_id} ({s_obj['name']})")

print(f"Total strains after Compound Genetics integration: {len(strains_map)}")
...
print(f"✅ Saved Compound Genetics database to data.js ({len(final_code):,} bytes)")
