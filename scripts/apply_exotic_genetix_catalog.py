...
from fetch_exotic_genetix_catalog import EXOTIC_GENETIX_CATALOG
...
for exg_strain in EXOTIC_GENETIX_CATALOG:
    s_obj = {k: v for k, v in exg_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded Exotic Genetix strain: {s_id} ({s_obj['name']})")

print(f"Total strains after Exotic Genetix integration: {len(strains_map)}")
...
print(f"✅ Saved Exotic Genetix database to data.js ({len(final_code):,} bytes)")
