...
from fetch_ethos_genetics_catalog import ETHOS_GENETICS_CATALOG
...
for eth_strain in ETHOS_GENETICS_CATALOG:
    s_obj = {k: v for k, v in eth_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded Ethos Genetics strain: {s_id} ({s_obj['name']})")

print(f"Total strains after Ethos Genetics integration: {len(strains_map)}")
...
print(f"✅ Saved Ethos Genetics database to data.js ({len(final_code):,} bytes)")
