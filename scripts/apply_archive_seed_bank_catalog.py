...
from fetch_archive_seed_bank_catalog import ARCHIVE_SEED_BANK_CATALOG
...
for arc_strain in ARCHIVE_SEED_BANK_CATALOG:
    s_obj = {k: v for k, v in arc_strain.items() if k != 'query'}
    s_id = s_obj["id"]
    ...
    print(f"✅ Embedded Archive Seed Bank strain: {s_id} ({s_obj['name']})")

print(f"Total strains after Archive Seed Bank integration: {len(strains_map)}")
...
print(f"✅ Saved Archive Seed Bank database to data.js ({len(final_code):,} bytes)")
